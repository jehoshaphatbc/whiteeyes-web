...
const activePostForMenu = ref<BlogPost | null>(null)
const dropdownPos = ref({ top: 0, left: 0 })

// Close dropdown on click outside or scroll
const closeDropdown = () => {
  activeDropdownId.value = null
  activePostForMenu.value = null
}

const toggleDropdown = (postItem: BlogPost, event: MouseEvent) => {
  if (activeDropdownId.value === postItem.id) {
    closeDropdown()
    return
  }
  ...
  // Calculate fixed position: right aligned with button
  dropdownPos.value = {
    top: rect.bottom + 6,
    left: Math.max(16, rect.right - 192),
  }
  activePostForMenu.value = postItem
  activeDropdownId.value = postItem.id
}
...
<template>
  <div class="space-y-6">
    ...
    <!-- Teleported Floating Action Dropdown Menu -->
    <Teleport to="body">
      <div
        v-if="activeDropdownId !== null && activePostForMenu"
      ...
      >
        <div class="py-1">
          <NuxtLink
            v-if="activePostForMenu.is_published"
            :to="`/blog/${activePostForMenu.slug}`"
            target="_blank"
            @click="closeDropdown"
          ...
          <button
            @click="openEditModal(activePostForMenu); closeDropdown()"
          ...
          <button
            @click="togglePublish(activePostForMenu); closeDropdown()"
            class="w-full text-left flex items-center gap-2.5 px-4 py-2.5 text-gray-300 hover:text-white hover:bg-void transition-colors"
          >
            <svg class="w-3.5 h-3.5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
            </svg>
            <span>{{ activePostForMenu.is_published ? 'UNPUBLISH (DRAFT)' : 'PUBLISH POST' }}</span>
          </button>
        </div>

        <div class="py-1">
          <button
            @click="openDeleteModal(activePostForMenu); closeDropdown()"
