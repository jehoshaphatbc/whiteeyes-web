...
const fetchReleases = async () => {
  loading.value = true
  try {
    releases.value = await $fetch<Release[]>(`${config.public.apiBase}/discography`)
  } catch (e) {
    console.error(e)
  } finally {
    loading.value = false
...
const save = async () => {
  try {
    if (form.value.id) {
      await $fetch(`${config.public.apiBase}/admin/discography/${form.value.id}`, {
        method: 'PUT',
        body: form.value,
      })
      toast.value = { show: true, message: 'Release updated successfully', type: 'success' }
    } else {
      await $fetch(`${config.public.apiBase}/admin/discography`, {
        method: 'POST',
        body: form.value,
      })
    ...
    toast.value = { show: true, message: err.data?.error || 'Failed to save release', type: 'error' }
  }
}
...
const executeDelete = async () => {
  ...
  try {
    await $fetch(`${config.public.apiBase}/admin/discography/${deletingId.value}`, { method: 'DELETE' })
    toast.value = { show: true, message: 'Release deleted', type: 'success' }
    showConfirm.value = false
    deletingId.value = null
    await fetchReleases()
  } catch (err: any) {
    toast.value = { show: true, message: err.data?.error || 'Failed to delete release', type: 'error' }
  }
}

const setFeatured = async (id: number) => {
  try {
    await $fetch(`${config.public.apiBase}/admin/discography/${id}/featured`, { method: 'PUT' })
    toast.value = { show: true, message: 'Set as featured release', type: 'success' }
    await fetchReleases()
  } catch (err: any) {
    toast.value = { show: true, message: err.data?.error || 'Failed to set featured', type: 'error' }
  }
}
</script>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-2xl w-full space-y-6 max-h-[90vh] overflow-y-auto">
          ...
          <form @submit.prevent="save" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <AdminImageUpload v-model="form.cover_art_url" label="COVER ARTWORK" subfolder="releases" />
            </div>
