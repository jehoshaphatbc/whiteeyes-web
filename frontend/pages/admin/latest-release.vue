...
onMounted(async () => {
  try {
    const data = await $fetch<Release>(`${config.public.apiBase}/latest-release`)
...
const save = async () => {
  ...
  try {
    let res: Release
    if (form.value.id) {
      res = await $fetch<Release>(`${config.public.apiBase}/admin/discography/${form.value.id}`, {
        method: 'PUT',
        body: { ...form.value, is_featured: true },
      })
    } else {
      res = await $fetch<Release>(`${config.public.apiBase}/admin/discography`, {
        method: 'POST',
        body: { ...form.value, is_featured: true },
      })
    }
    ...
    toast.value = { show: true, message: err.data?.error || 'Failed to save featured release', type: 'error' }
  } finally {
    saving.value = false
  }
}
