...
const fetchItems = async () => {
  loading.value = true
  try {
    items.value = await $fetch<MerchItem[]>(`${config.public.apiBase}/merch`)
  } catch (e) {
    console.error(e)
  } finally {
    loading.value = false
...
const save = async () => {
  try {
    if (form.value.id) {
      await $fetch(`${config.public.apiBase}/admin/merch/${form.value.id}`, {
        method: 'PUT',
        body: form.value,
      })
      toast.value = { show: true, message: 'Merchandise item updated', type: 'success' }
    } else {
      await $fetch(`${config.public.apiBase}/admin/merch`, {
        method: 'POST',
        body: form.value,
      })
    ...
    toast.value = { show: true, message: err.data?.error || 'Failed to save merchandise item', type: 'error' }
  }
}
...
const executeDelete = async () => {
  ...
  try {
    await $fetch(`${config.public.apiBase}/admin/merch/${deletingId.value}`, { method: 'DELETE' })
    toast.value = { show: true, message: 'Merchandise item deleted', type: 'success' }
    showConfirm.value = false
    deletingId.value = null
    await fetchItems()
  } catch (err: any) {
    toast.value = { show: true, message: err.data?.error || 'Failed to delete merchandise item', type: 'error' }
  }
}
</script>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
        <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-lg w-full space-y-4">
          ...
          <form @submit.prevent="save" class="space-y-4">
            <AdminImageUpload v-model="form.image_url" label="PRODUCT IMAGE" subfolder="merch" />
