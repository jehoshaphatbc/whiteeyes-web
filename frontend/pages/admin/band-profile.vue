...
onMounted(async () => {
  try {
    const data = await $fetch<BandProfile>(`${config.public.apiBase}/band-profile`)
...
const save = async () => {
  saving.value = true
  try {
    const res = await $fetch<BandProfile>(`${config.public.apiBase}/admin/band-profile`, {
      method: 'PUT',
      body: form.value,
    })
    form.value = res
    ...
    toast.value = { show: true, message: err.data?.error || 'Failed to save band profile', type: 'error' }
  } finally {
    saving.value = false
  }
}
