...
onMounted(async () => {
  try {
    const data = await $fetch<About>(`${config.public.apiBase}/about`)
...
const save = async () => {
  saving.value = true
  try {
    const res = await $fetch<About>(`${config.public.apiBase}/admin/about`, {
      method: 'PUT',
      body: form.value,
    })
    form.value = res
    ...
    toast.value = { show: true, message: err.data?.error || 'Failed to save about section', type: 'error' }
  } finally {
    saving.value = false
  }
}
